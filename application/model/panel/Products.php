...
{ 
    $query = "SELECT products.*, categories.name AS categoryname, GROUP_CONCAT(DISTINCT product_images.image_url) AS image_urls, GROUP_CONCAT(DISTINCT colors.hex_value) AS colors, (SELECT SUM(stock) FROM colors WHERE colors.product_id = products.product_id) AS total_quantity FROM products LEFT JOIN categories ON products.category_id = categories.category_id LEFT JOIN product_images ON products.product_id = product_images.product_id LEFT JOIN colors ON products.product_id = colors.product_id GROUP BY products.product_id, categories.name ORDER BY products.product_id DESC;";
    
    $result = $this->query($query)->fetchAll();
    $this->closeConnection();
    return $result;
}
    public function allPanel_for_print()
{ 
    $query = "SELECT products.*, categories.name AS categoryname, GROUP_CONCAT(DISTINCT product_images.image_url) AS image_urls, GROUP_CONCAT(DISTINCT colors.titel_name) AS colors, (SELECT SUM(stock) FROM colors WHERE colors.product_id = products.product_id) AS total_quantity FROM products LEFT JOIN categories ON products.category_id = categories.category_id LEFT JOIN product_images ON products.product_id = product_images.product_id LEFT JOIN colors ON products.product_id = colors.product_id GROUP BY products.product_id, categories.name ORDER BY products.product_id DESC;";
...
//!-------------------------------------------------------------------colors 
public function getProductColorsWithStock() {
    $query = "SELECT 
                product_id, 
                titel_name, 
                stock 
...
{
    $query = "INSERT INTO `colors`(`product_id`, `color_name`, `titel_name`, `hex_value`, `stock` ,`created_at`) VALUES (?,?,?,?,?,NOW())";
    $params = array_merge([$id] ,array_values($values) );  
    $this->execute($query, $params);  
    $this->closeConnection();  
}
