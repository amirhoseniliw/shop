    ...
    <table class="table table-bordered table-striped text-center align-middle">
        <thead class="table-light">
            <tr>
                <th>تاریخ</th>
                <th>تعداد سفارش</th>
                <th>مقدار فروش (تومان)</th>
            </tr>
        </thead>
        <tbody>
            <?php
            if (!empty($countByDate)) {
                // پیدا کردن بیشترین مقدار فروش
                $maxSale = max(array_column($countByDate, 'total_paid'));

                foreach ($countByDate as $row) {
                   $date = $this->jalaliData($row['order_date']);
                    $totalOrders = $row['total_orders'];
                    $totalPaid = $row['total_paid'];

                    // محاسبه درصد نسبت به بیشترین فروش
                    $ratio = $maxSale > 0 ? $totalPaid / $maxSale : 0;

                    // تولید رنگ بین قرمز تا سبز
                    $red = (int)(255 * (1 - $ratio));
                    $green = (int)(255 * $ratio);
                    $bgColor = "rgb($red, $green, 100)";
                    echo "
                        <tr>
                            <td>{$date}</td>
                            <td>{$totalOrders}</td>
                ...
                echo '<tr><td colspan="3">داده‌ای برای نمایش وجود ندارد.</td></tr>';
            }
            ?>
        </tbody>
    </table>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <table class="status-table">
                    ...
                    <tbody>
                        <tr class="paid_orders">
                            <td>پرداخت شده </td>
                            <td><?= $orders['paid_orders'] ?></td>
                            <td><?= $orders['paid_percentage'] ?>%</td>
                        </tr>
                        <tr class="intransit_orders">
                            <td>در حال ارسال </td>
                            <td><?= $orders['intransit_orders'] ?></td>
                            <td><?= $orders['intransit_percentage'] ?>%</td>
                        </tr>
                        <tr class="delivered_orders">
                            <td>تحول داده شده </td>
                            <td><?= $orders['delivered_orders'] ?></td>
                            <td><?= $orders['delivered_percentage'] ?>%</td>
                        </tr>
                        <tr class="canceled_orders">
                            <td>لغو شده</td>
                            <td><?= $orders['canceled_orders'] ?></td>
                            <td><?= $orders['canceled_percentage'] ?>%</td>
                        </tr>
                        <div style="text-align: center; color:rgb(44, 40, 167);">
                            <b>
                                <h4>کل سفارشات </h4>
