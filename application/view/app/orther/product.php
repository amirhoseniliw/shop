...
$title = $post['name'];
$this->include('app.layouts.header' , compact('categories' , 'title')); ?>
<title>t</title>
...
if ($favorites !== null) {
    foreach ($favorites as $favorite) {
        if ($post['product_id'] == $favorite['product_id']) {
            $found = true;
            break;
        }
    }
...
?>
  ...
  data-url="https://tahrirkhayam.ir/product/add_favorites/<?= $post['product_id'] ?>"
  data-is-fav="<?= $found ? '1' : '0' ?>"
  data-bs-toggle="tooltip"
  data-bs-placement="top"
  data-bs-title="<?= $found ? 'حذف از علاقه‌مندی' : 'افزودن به علاقه‌مندی' ?>"
                            ...
                            <div class="product-meta-color-items">
                               ...
                               $color_id = 'option' . $i; 
                                ...
                                <input type="radio" class="btn-check" name="colors" id="<?php echo $color_id; ?>"  
                                    autocomplete="off" value="<?php echo $color['color_id']; ?> "<?php if($color['hex_value'] == 'hue' || $i == 0) { echo('checked');} ?>>
